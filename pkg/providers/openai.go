...
func init() {
	if err := gollm.RegisterProvider("custom-openai", newOpenAIClientFactory); err != nil {
...
func newOpenAIClientFactory(ctx context.Context, _ *url.URL) (gollm.Client, error) {
...
var _ gollm.Client = &OpenAIClient{}
...
func (c *OpenAIClient) StartChat(systemPrompt, model string) gollm.Chat {
...
func (c *OpenAIClient) GenerateCompletion(ctx context.Context, req *gollm.CompletionRequest) (gollm.CompletionResponse, error) {
...
func (c *OpenAIClient) SetResponseSchema(schema *gollm.Schema) error {
...
func (cs *openAIChatSession) Send(ctx context.Context, contents ...any) (gollm.ChatResponse, error) {
...
func (cs *openAIChatSession) SendStreaming(ctx context.Context, contents ...any) (gollm.ChatResponseIterator, error) {
	...
	var yielded bool
	iteratorFunc := func(yield func(gollm.ChatResponse, error) bool) {
		if !yielded {
			yielded = true
...
var _ gollm.ChatResponse = (*openAIChatResponse)(nil)
...
func (r *openAIChatResponse) Candidates() []gollm.Candidate {
	if r.openaiCompletion == nil {
		return nil
	}
	candidates := make([]gollm.Candidate, len(r.openaiCompletion.Choices))
...
var _ gollm.Candidate = (*openAICandidate)(nil)

func (c *openAICandidate) Parts() []gollm.Part {
...
var _ gollm.Part = (*openAIPart)(nil)

func (p *openAIPart) AsText() (string, bool) {
	return p.content, p.content != ""
}

func (p *openAIPart) AsFunctionCalls() ([]gollm.FunctionCall, bool) {
	if len(p.toolCalls) == 0 {
		return nil, false
	}

	gollmCalls := make([]gollm.FunctionCall, len(p.toolCalls))
	for i, tc := range p.toolCalls {
		...
		gollmCalls[i] = gollm.FunctionCall{
